...
async function analyzePR(pr) {
  ...
  const allCurrentVersions = {
    ...currentVersions.dependencies,
    ...currentVersions.devDependencies,
  };
  ...
  for (const [packageName, prVersion] of Object.entries(prVersions)) {
    ...
    if (!currentVersion) {
      console.log(
        `  ℹ️  ${packageName}: Not in current package.json (may have been removed)`,
      );
      obsoletePackages.push({
        packageName,
        prVersion,
        currentVersion: 'removed',
    ...
    if (comparison < 0) {
      console.log(
        `  ❌ ${packageName}: PR version ${prVersion} < current ${currentVersion}`,
      );
      obsoletePackages.push({
        packageName,
        prVersion,
      ...
      console.log(
        `  ℹ️  ${packageName}: PR version ${prVersion} = current ${currentVersion}`,
      );
  ...
  return {
    ...
    prVersions,
    reason: isObsolete
      ? 'All packages obsolete or equal'
      : 'Has valid upgrades',
  };
}
...
async function findRecentUpgradePR() {
  ...
  const closedPRs = await githubRequest(
    `/repos/${REPO_OWNER}/${REPO_NAME}/pulls?state=closed&per_page=50`,
  );

  // Look for Angular upgrade PR
  const upgradePR = closedPRs.find(
    (pr) =>
      pr.merged_at &&
...
function generateObsoleteComment(pr, analysis, upgradePR) {
  ...
  for (const pkg of analysis.obsoletePackages) {
    const status =
      pkg.reason === 'Lower version'
        ? '⬇️ Older'
        : pkg.reason === 'Same version'
          ? '➡️ Same'
          : '🗑️ Removed';
...
async function main() {
  ...
  if (!GITHUB_TOKEN) {
    ...
    console.error(
      'Please set GITHUB_TOKEN with a valid GitHub Personal Access Token',
    );
    process.exit(1);
  }
