...
| `develop`  | ACTIVE (integration) | GitFlow-style long-term integration branch. Accepts PRs from feature branches. CI deliberately skipped on push to allow day-to-day work without blocking; CI runs on pull_request (including Dependabot PRs for dependency updates). Use `workflow_dispatch` to run CI on-demand. Used to migrate the old `master` codebase to the modern pattern (Angular 17+ standalone components, NestJS services, Telegram auth, Docker). Permanent for future feature integration workflows. |
...
`main` is kept in sync with `develop` via fast-forward at release time. `skeleton` is the immutable baseline; `main` branches from it and periodically syncs releases back from `develop`. CI runs on pushes to `main` and `skeleton` (all events: push, pull_request, workflow_dispatch). Pushes to `develop` deliberately skip CI to allow day-to-day work without blocking; pull requests into `develop` run CI automatically, including Dependabot PRs for dependency updates. Use `workflow_dispatch` to run CI on-demand on any branch before a release cut.
...
- **Dependabot** (`.github/dependabot.yml`) proposes upgrades as exact-pin PRs into `develop`, grouped by ecosystem (Angular, NestJS, Tailwind, dev-tools, security, other). Security and version updates arrive in separate PR streams. Nothing automerges.
