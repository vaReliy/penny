...
**Plan-back checkpoint (every T2/T3 implementation dispatch)**: The orchestrator owns the outcome of the full cycle, not just the routing of it. Every T2/T3 implementation dispatch prompt must require the agent to reply with a ≤10-line plan — mechanism, files it expects to touch, and the exact verification command with its expected result — and to wait for approval before writing code. The orchestrator judges that plan **only** against what it already holds (the task file and the rules cited in the dispatch prompt), never by reading source, then approves, corrects once, or re-routes to a different agent. One correction round maximum, then proceed. T0/T1 skip this. Full criteria, bounds, and the re-routing case: `rules/local/workflow.md` § "Phase 2.5 — Plan-back checkpoint before implementation (T2/T3)".
