...
## 2026-07-11 — workflow: a task file in `done/` is not evidence of completion — verify acceptance criteria against git history before trusting it

Why: The 2026-07-11 re-review found `2026-07-07-09-infra-security-hardening.md` in `tasks/done/` with **zero implementing commits**: `git log --all` over the files it targets shows no SHA-pinning, no pr-management.yml change, no `--ignore-scripts`, no `proxy_hide_header` — and no METRICS.md row exists for it (the METRICS Stop-hook check landed only after this task's window, so nothing caught the ghost-done). The move to `done/` itself leaves no git trace because `docs/rebuild/` is excluded via `.git/info/exclude`, so a misfiled `mv` is invisible. Rule: before a milestone close (or any decision that treats `done/` as ground truth), sample done-tasks whose acceptance criteria are cheaply greppable (`grep "uses:.*@v[0-9]" .github/workflows/` here would have failed instantly) and cross-check each against METRICS.md rows + an implementing commit. Related: two more this-week findings share the same shape — a config comment claiming "CI is still gated" while CI never runs the target, and an inbox entry recording a serve-static fix that isn't in the working tree. Written claims of enforcement (task moves, comments, inbox entries) must be verified against the artifact, not trusted transitively.
Belongs in (guess): rules/workflow.md (milestone close / DoD-audit step — extend the existing plan-vs-reality rule) | claude-ts-upstream (cts-rule-auditor or a done-move checklist)

## 2026-07-11 — mongoose: schema `default:` values apply to legacy documents missing the field, no migration needed

Why: Task `2026-07-09-02-admin-role-assignment-and-jwt-issuance` added `roles: string[]` (default `[]`) to the `User` Typegoose schema/mapper without a migration script, on the assumption that existing Mongo documents (which predate the field) would still hydrate safely. Confirmed by `reviewer` and `tester`: Mongoose/Typegoose applies a schema's `default:` value to any document hydrated via a normal query (`find`, `findOne`, etc.) whose BSON is missing that path — but only when the query does NOT use `.lean()` (`.lean()` returns raw BSON with no schema-default application). This repo's `mongo-user-repository.ts` never uses `.lean()`, so `UserMapper.toDomain` never sees an `undefined roles` for legacy documents. General rule: adding a new field with a `default:` to an existing Typegoose/Mongoose collection is migration-free only if every read path for that model avoids `.lean()`; if any repository method uses `.lean()`, that path needs either a migration or an explicit `?? []`-style fallback in the mapper.
Belongs in (guess): rules/architecture-backend.md (MongoDB/Mongoose patterns section) or rules/migrations-queue.md

## 2026-07-10 — security: don't justify accepting a cookie-gated timing side-channel with "the requester already knows their own state"

Why: Task `2026-07-10-01-session-guard-timing-and-cookie-signal` made `res.clearCookie(...)` unconditional across all three `AuthenticationError` branches in `apps/api/src/auth/session.guard.ts`, but left a timing difference (missing-cookie short-circuits before JWT verify/DB lookup) as an accepted risk. The first drafted justification — "the attacker already knows whether they sent a cookie" — was flagged by `security-scanner` as unsound: it only holds for a _direct_ attacker calling the API with their own headers, not for the classic XS-Leak "session oracle" scenario where a cross-site attacker page forces the _victim's browser_ to send the request, and the attacker does not control/know whether the cookie was attached (the browser decides). The correct mitigating argument, confirmed by cross-checking `apps/api/src/auth/auth.controller.ts`, is that `AUTH_COOKIE_NAME`/`XSRF_COOKIE_NAME` are set with `sameSite: 'lax'`, which prevents the cookie from being attached to cross-site fetch/XHR requests in the first place — closing the practical timing-oracle vector and leaving only a much harder residual (top-level navigation + browser-isolated timing measurement). Any future accepted-risk writeup for a cookie-gated timing/behavior difference must reason from the actual cookie/CORS/SameSite control, not from what "the attacker already knows."
Belongs in (guess): rules/validation-authorization.md (timing side-channel / XS-Leak reasoning section)

## 2026-07-10 — sub-floor polish (deferred): `session.guard.ts`'s three `AuthenticationError` branches now each duplicate an identical `clearCookie(AUTH_COOKIE_NAME, ...)` + `clearCookie(XSRF_COOKIE_NAME, ...)` pair

Why: Task `2026-07-10-01-session-guard-timing-and-cookie-signal` added a third occurrence of this pair (missing-cookie branch) to close a Set-Cookie header asymmetry; `reviewer` flagged the resulting 3x duplication as a DRY candidate (private `clearAuthCookies(res)` helper) but rated it cosmetic/sub-floor, not a fix-now item. Left as-is; worth folding into the next `session.guard.ts` touch rather than a standalone task.
Belongs in (guess): fold into next `session.guard.ts` change, or `rules/code-style-backend.md` if this pattern recurs elsewhere.

## 2026-07-10 — testing: opacity-assertion regexes must exclude words present in the shared error's own default message

Why: Task `2026-07-08-02-session-guard-full-opaque-401` aligned the last two `AuthenticationError` throw sites in `apps/api/src/auth/session.guard.ts` to the no-arg opaque form. A new spec test asserted the caught message didn't match `/required|missing|cookie/i` — but `AuthenticationError`'s own `DEFAULT_MESSAGE` (`libs/shared/errors/src/lib/authentication-error.ts`) is `'Authentication is required or has failed.'`, which contains "required", so the test self-failed against the intentionally-opaque default it was supposed to allow. Fixed by dropping "required" from that one regex. Any future opacity/leak-check test using `not.toMatch(/word1|word2/i)` against a `BaseError` subclass's default message must first check that no word in the negative-match list appears in the class's own `DEFAULT_MESSAGE` constant.
Belongs in (guess): rules/testing.md (opacity/error-message test-writing section)
