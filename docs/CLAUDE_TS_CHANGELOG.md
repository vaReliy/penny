...
## 2026-07-16 — Fix: `cts-sync.sh` doesn't persist or compare the resolved source across runs, so switching between default (URL) and `--source <local-path>` looks like an upstream regression

- **Component**: `.claude/scripts/cts-sync.sh` (source resolution, `.cts-version` writing), `.claude/skills/cts-update/SKILL.md` (step 2/3)
- **Type**: Fix
- **What happened**: A plain `/cts-update` (no `--source`/`--branch`) resolved to the default `https://github.com/vaReliy/claude-ts.git` (`origin/main`, dated 2026-07-01) and produced a 30-file diff that read as a mass regression — large rules deletions, `CLAUDE.md` reverting its Angular-only routing table, several skills reported "removed upstream." First suspected an upstream history rewrite (the old `.cts-version` ref was unreachable via `git cat-file -t` in `~/.cache/claude-ts` even after `git fetch`) — that theory was wrong and retracted after investigation. The actual cause: the prior sync had been run with `--source <path-to-a-separate-local-claude-ts-checkout>` pointing at an unpushed/WIP feature branch 13 commits ahead of `origin/main`. `cts-sync.sh` records only the resolved commit SHA in `.cts-version`, never which source/branch produced it, so a later run against a _different_ source with an _older_ tip than the previously-used one is indistinguishable, from the script's and the user's perspective, from upstream actually losing commits. Confirmed by re-running with the same explicit `--source`: `NEW_SHA` matched `OLD_SHA` exactly, zero diff.
- **Why it matters upstream**: Any claude-ts consumer or maintainer who sometimes points `--source` at a local checkout (e.g. for testing pre-merge branches, or for a fork) and sometimes runs the plain default risks the exact same false-regression scare — indistinguishable from real data loss without manually digging through both repos' histories, as this session had to.
- **Suggested upstream change**: Have `cts-sync.sh` write a companion `.cts-source` file (e.g. `<resolved-origin-url-or-path> <branch-or-HEAD-SHA-of-checkout>`) alongside `.cts-version` on every successful run. On the next run, if the newly-resolved source/branch differs from what's recorded and no explicit `--source`/`--branch` was passed to override it, print a warning before touching any files (e.g. "Last sync used `<recorded>`; this run resolved `<new>` — these differ. Pass `--source`/`--branch` to match, or continue to switch.") rather than silently proceeding as if it's a normal incremental update. As an interim mitigation until the engine-level `.cts-source` check exists, a "Triage note (source mismatch)" was added to `.claude/skills/cts-update/SKILL.md` step 3, alongside the existing contribution-round-trip triage note, telling the assistant to suspect a source mismatch (not real upstream data loss) when a plain-default run produces a mass-deletion diff, and to verify by re-running with the suspected prior `--source`/`--branch` and checking for a no-op.
- **Status**: pending-port
