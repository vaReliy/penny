...
- **`libs/budget/feature-planner`** (new Nx lib, `scope:budget`/`type:feature`/`platform:web`) — `PlannerPageComponent` (standalone, Angular 17+, signal inputs) composing the category list and month selector against signal stores. Loading / ready / error states via `BudgetRequestState` delegation, all strings via Transloco `budget` scope (uk).
- Tests: 65 unit tests (`feature-planner`, `budget-ui` updates), Playwright e2e (`apps/web-e2e/src/planner-page.spec.ts`, 3-browser matrix), all green.
- Gate: tester → reviewer → qa, 2 restart cycles. Reviewer cycle 1 found `goToPreviousMonth()`/`goToNextMonth()` didn't clear `editingCategoryId()` on month nav (in-progress edits silently discarded/misattributed); fixed by impl, verified clean cycle 2. QA (chrome-devtools MCP substituted for Playwright; orchestrator manual walkthrough) found categories with spend but zero budget rendered empty bar (0% fill, no color), misleadingly reading "on track" instead of overspend — this bug invisible to automated tests and code-only review, only surfaced by rendered-page inspection; fixed by impl, verified clean cycle 3, re-confirmed live in browser. 0 emitted; all acceptance criteria met (parity+, month attribution, edge states, lint/typecheck/test green).
...
- **`rules/nx-generators.md`** — dropped a dangling private decision-ID reference on the `.js`-extension-enforcement note; now points at `DECISIONS.md` ADR-005, the committed record of that decision.
