...
- `ensured ledger merge strategy in: ./.gitattributes` — the CTS-managed `merge=union` block for the append-only ledgers was written or repaired. Expected and silent on most runs (the engine writes nothing when the block is already current); on a project's **first** update after this feature shipped it will always appear. Narrate it once as a one-liner: parallel branches appending to `docs/KNOWLEDGE_INBOX.md`, `docs/METRICS.md`, `docs/CLAUDE_TS_CHANGELOG.md`, and `CHANGELOG.md` now merge without conflicts. A companion `GITATTRIBUTES NOTE:` line means the consumer already had their own rule for one of those paths — surface it, since the CTS block is appended last and therefore wins.
