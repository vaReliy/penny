...
import { ArchiveCategoryService } from './archive-category.service.js';
...
describe('ArchiveCategoryService', () => {
  ...
  beforeEach(() => {
    ...
    service = new ArchiveCategoryService({ categoryRepository: repository });
  });
  ...
  it('archives an existing category', async () => {
    ...
    expect(outcome.data.isArchived()).toBe(true);
  ...
  it('throws AuthenticationError for a non-active caller', async () => {
    ...
    await expect(
      service.run({ id: CATEGORY_ID }, buildContext(PENDING_CALLER)),
    ).rejects.toBeInstanceOf(AuthenticationError);
  });

  it('throws NotFoundError when the category does not exist in the workspace', async () => {
    await expect(
      service.run({ id: UNKNOWN_CATEGORY_ID }, buildContext(ACTIVE_CALLER)),
    ).rejects.toBeInstanceOf(NotFoundError);
  });

  it('throws a DomainError conflict when the category is already archived', async () => {
    repository.seed(
      Category.create(CATEGORY_ID, 'ws-1', 'Groceries').archive(new Date()),
    );

    const error = await service
      .run({ id: CATEGORY_ID }, buildContext(ACTIVE_CALLER))
  ...
  it('never hard-deletes: repository.delete is never invoked', async () => {
    repository.seed(Category.create(CATEGORY_ID, 'ws-1', 'Groceries'));
    const deleteSpy = repository.delete.bind(repository);
    let deleteCalled = false;
    repository.delete = async (id: string) => {
      deleteCalled = true;
      return deleteSpy(id);
    };

    await service.run({ id: CATEGORY_ID }, buildContext(ACTIVE_CALLER));
