...
Amounts travel as `SerializedMoney` (integer minor units encoded as a string, precision-safe for transport) on responses, or as a plain minor-units integer (`amountMinorUnits`) on write requests, paired with LIVR validation in `budget-validation`. Dates travel as ISO strings (`YYYY-MM-DD` for calendar dates, `YYYY-MM` for budget months).
